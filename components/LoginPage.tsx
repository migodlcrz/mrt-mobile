...
const LoginPage: React.FC<LoginProps> = ({navigation}) => {
  ...
  return (
    <LinearGradient colors={['#dbe7c9', '#107d66', '#035443']}>
      <View className=" h-full justify-center items-center">
        <View className="flex space-y-4 h-auto items-center justify-center">
          <View className="flex flex-col items-center">
            <Image
              source={require('../images/trainlogo.png')}
              className="w-[300px] h-[130px]"
              // style={{width: 100, height: 100}} // Adjust the width and height as needed
            />

            <Text className=" text-black font-bold text-2xl">Welcome User</Text>
          </View>
          <View className="space-y-4 items-center">
            <TouchableOpacity
              ...
              <TextInput
                className="pin1 bg-slate-300 border-[#0d9276] border-2 px-3 w-14 py-2 rounded-lg text-black text-3xl text-center font-bold"
                placeholderTextColor="#BDBDBD"
                readOnly
                value={pin[0] || ''}
              />
              <TextInput
                className="pin2 bg-slate-300 border-[#0d9276] border-2 px-3 w-14 py-2 rounded-lg text-black text-3xl text-center font-bold"
                placeholderTextColor="#BDBDBD"
                readOnly
                value={pin[1] || ''}
              />
              <TextInput
                className="pin3 bg-slate-300 border-[#0d9276] border-2 px-3 w-14 py-2 rounded-lg text-black text-3xl text-center font-bold"
                placeholderTextColor="#BDBDBD"
                readOnly
                value={pin[2] || ''}
              />
              <TextInput
                className="pin4 bg-slate-300 border-[#0d9276] border-2 px-3 w-14 py-2 rounded-lg text-black text-3xl text-center font-bold"
                placeholderTextColor="#BDBDBD"
                readOnly
                value={pin[3] || ''}
              />
            ...
            {!showKeyboard && (
              <View>
                <Text className="text-black">{pinMessage}</Text>
              </View>
            )}
            {showKeyboard && (
              <View className="flex flex-col space-y-4 w-[200px] items-center">
                ...
                <View className="flex flex-row space-x-4 w-full justify-end">
                  ...
                  <TouchableOpacity
                    ...
                    className=" p-3 rounded-full h-14 w-14 justify-center items-center">
        ...
        <LinearGradient
          colors={['#6dad8a', '#035443']}
          className="bg-[#0d9276] p-2 w-full absolute bottom-0 items-center justify-center ">
